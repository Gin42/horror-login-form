...
document.querySelector('.visible').addEventListener('change', (e) => {
    document.querySelectorAll('.visible-icon').forEach(element => element.style.display = element.style.display === 'none' ? 'block' : 'none');
...
function updateMask(increment) {
    ...
    document.querySelectorAll('.tear').forEach(element=> {
        element.setAttribute('mask', 'url(#' + 'maskPos' + (pos++) + 'Fr' + currentFrame + ')');
    });
    pos = 1;
}
